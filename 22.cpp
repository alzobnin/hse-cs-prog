// Продолжаем говорить про идиому RAII ("Выделение ресурса есть инициализация")
// Рассмотрим сегодня хрестоматийный пример ресурса: файл. (Другой пример - память - мы уже знаем.)
...
// Отдельное полезное упражнение - написать класс, в котором конструкторы, деструкторы и оператор присваивания
// пишут сообщения о том, что их вызвали (с указанием адреса текущего объекта),
// а потом проанализировать вывод программы при использовании этого класса, например, с вектором.
...
// Конструкции long(this) и long(&other) - неявные приведения указателя (адреса) к длинному целому числу

int main() {
    std::vector<C> v(3);  // трижды вызовется конструктор по умолчанию
    v.resize(5);  // скорее всего, произойдёт реаллокация:
                  // сначала три старых элемента скопируются,
                  // затем сконструируются два новых,
                  // потом три старых элемента удалятся
    std::cout << "Hello, world\n";
} // а тут будут вызваны деструкторы для всех пяти элементов вектора
